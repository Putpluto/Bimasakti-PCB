...
# ]
input_columns = ['Timestamp','EngineTemp','BatteryVoltage','VGear','RPM','Suspension1','Suspension2','Suspension3',
...
def process_dataframe(df):
    ...
    try:
        # Create a copy and ensure all columns exist
        processed = pd.DataFrame(index=df.index)
        
        # Copy input columns first
        for col in input_columns:
            processed[col] = df[col] if col in df.columns else 0.0
        ...
        for col in ['Suspension1', 'Suspension2', 'Suspension3', 'Suspension4']:
            processed[col] = (((processed[col] - 17785)/(-227.97))+7)
            if len(processed) > S2:
                processed[col] = processed[col] - processed[col].iloc[S1:S2].mean()
            # if len(processed) >= N:
            #     processed[col] = medfilt(processed[col].astype(np.float64), kernel_size=N)

        # Zero out RPM outliers
        rpm_cols = ['RPM_FrontRight', 'RPM_FrontLeft', 'RPM_RearRight', 'RPM_RearLeft']
        for col in rpm_cols:
            mask = processed[col] > 1370
            processed.loc[mask, col] = 0

        # Initialize CMA columns
        for source, target in [
            ('RPM_FrontRight', 'CMA_RPM_FrontRight'),
            ('RPM_FrontLeft', 'CMA_RPM_FrontLeft'),
            ('RPM_RearRight', 'CMA_RPM_RearRight'),
            ('RPM_RearLeft', 'CMA_RPM_RearLeft')
        ]:
            processed[target] = processed[source]

        # Apply filters if enough data
        if len(processed) >= N:
            for col in ['CMA_RPM_FrontRight', 'CMA_RPM_FrontLeft', 
                       'CMA_RPM_RearRight', 'CMA_RPM_RearLeft']:
                values = processed[col].to_numpy(dtype=np.float64)
                processed[col] = medfilt(values, kernel_size=N)

                # Apply Savitzky-Golay if possible
        ...
        return pd.DataFrame(0, index=df.index, columns=columns)
...
def update_plot_data():
    ...
    with data_lock:
        ...
        # Update the gui_data dictionary
        gui_data['plot_data'].update({
            'x_values': x_values,
            'y1_values': y1_values,
            'y2_values': y2_values,
            'y3_values': y3_values,
            ...
            while not stop_serial_event.is_set():
                try:
                    ...
                    if len(values) != len(input_columns):
                        print(f"Warning: Expected {len(input_columns)} input values, got {len(values)}")
                        continue

                    try:
                        row_data = {col: float(val) for col, val in zip(input_columns, values)}
                        ...
                        with data_lock:
                            if len(data) == 0:
                                ...
                                data = pd.concat([data, processed_row], ignore_index=True)
                            
                            if len(data) > 10000:
                                data = data.iloc[-5000:]
                            
                            if not logging_paused[0] and current_log_file:
                                df_row.to_csv(current_log_file, mode='a', header=False, index=False)
                                total_rows_logged[0] += 1
                        
                        update_plot_data()  # This has its own locking
...
def main():
    ...
    def gui():
        ...
        # Get local references to the plot data
        plot_data = gui_data['plot_data']
        
        # Safety check for slider value and data length
        with data_lock:
            data_length = len(data)
            if data_length > 0:
                if slider_value[0] >= data_length:
                    slider_value[0] = data_length - 1
                current_idx = slider_value[0]
                
                # Create local copies of needed data
                current_row = data.iloc[current_idx].copy()
            else:
                current_idx = 0
                current_row = None
        ...
        if play_mode[0] and (current_time - time_state["last_time"] > 1.0 / sample_rate[0]):
            slider_value[0] += 1
            if slider_value[0] >= len(plot_data['x_values']):
                slider_value[0] = len(plot_data['x_values']) - 1
                play_mode[0] = False
        ...
        if len(data) > 0:  # Only try to plot/show data if we have any
            ...
            # Plots, bars, etc...
            imgui.begin("Plot")
            if len(plot_data['x_values']) > 0:  # Check array lengths too
                if auto_follow[0]:
                ...
                if implot.begin_plot("Plot"):
                    implot.plot_line("Front Right RPM", plot_data['x_values'], plot_data['y1_values'])
                    implot.plot_line("Front Left RPM", plot_data['x_values'], plot_data['y2_values'])
                    implot.plot_line("Rear Right RPM", plot_data['x_values'], plot_data['y3_values'])
                    implot.plot_line("Rear Left RPM", plot_data['x_values'], plot_data['y4_values'])
                    implot.plot_line("Derajat", plot_data['x_values'], plot_data['y5_values'])
            ...
            imgui.text(f"Frame Time: {1000.0 / imgui.get_io().framerate:.1f} ms")
            imgui.end()
            ...
            if implot.begin_plot("RPM"):
                implot.plot_line("RPM", plot_data['x_values'], plot_data['y6_values'])
            ...
            if current_row is not None:
                ...
                derajat_value = current_row['Derajat']
                window_width = imgui.get_window_size().x - imgui.get_style().window_padding.x * 2
                if (derajat_value >= 0):
                    imgui.push_style_color(imgui.Col_.plot_histogram, (0.0, 0.0, 1.0, 1.0))
                else:
                    imgui.push_style_color(imgui.Col_.plot_histogram, (1.0, 0.0, 0.0, 1.0))
                ...
                for label in suspension_labels:
                    ...
                    value_bar(label, value, ImVec2(50, 400), min_value=-10, max_value=10, flags=ValueBarFlags.VERTICAL | ValueBarFlags.CENTER_ZERO)
                ...
                for label in ['CMA_RPM_FrontRight', 'CMA_RPM_FrontLeft', 
                            'CMA_RPM_RearRight', 'CMA_RPM_RearLeft']:
                ...
                draw_list = imgui.get_window_draw_list()
                target_speed = current_row['AverageSpeed']
                speedometer1.render(draw_list, target_speed)
                imgui.end()

                imgui.begin("GPS Speed")
                draw_list = imgui.get_window_draw_list()
                target_speed = current_row['GPSS']
