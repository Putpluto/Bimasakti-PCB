...
def process_dataframe(df):
    ...
    for col in columns:
        if col not in df.columns:
            df[col] = 0.0  # Initialize missing columns with zeros
    
    # Only process if we have enough data
    if len(df) < 2:
        return df
        
    N = min(17, len(df))  # Adjust kernel size based on data length
    ...
    try:
        ...
        for col in ['Suspension1', 'Suspension2', 'Suspension3', 'Suspension4']:
            df[col] = (df[col] - 26139)/-334.73
            if len(df) > S2:  # Only calculate mean if we have enough data
                df[col] = df[col] - df[col].iloc[S1:S2].mean()
            if len(df) >= N:
                df[col] = medfilt(df[col], kernel_size=N)

        # Zero out RPM outliers
        for col in ['RPM_FrontRight', 'RPM_FrontLeft', 'RPM_RearRight', 'RPM_RearLeft']:
            df.loc[df[col] > 1370, col] = 0
            
        # Create CMA columns (only if enough data)
        if len(df) >= N:
            df['CMA_RPM_FrontRight'] = medfilt(df['RPM_FrontRight'], kernel_size=N)
            df['CMA_RPM_FrontLeft'] = medfilt(df['RPM_FrontLeft'], kernel_size=N)
            df['CMA_RPM_RearRight'] = medfilt(df['RPM_RearRight'], kernel_size=N)
            df['CMA_RPM_RearLeft'] = medfilt(df['RPM_RearLeft'], kernel_size=N)

            # Apply Savitzky-Golay (only if enough data)
            window_length = min(11, len(df))
            if window_length > 2:  # Need at least 3 points for quadratic fit
                for col in ['CMA_RPM_FrontRight', 'CMA_RPM_FrontLeft', 
                           'CMA_RPM_RearRight', 'CMA_RPM_RearLeft']:
                    df[col] = savgol_filter(df[col], window_length=window_length, polyorder=2)
        else:
            # If not enough data, just copy the raw values
            df['CMA_RPM_FrontRight'] = df['RPM_FrontRight']
            df['CMA_RPM_FrontLeft'] = df['RPM_FrontLeft']
            df['CMA_RPM_RearRight'] = df['RPM_RearRight']
            df['CMA_RPM_RearLeft'] = df['RPM_RearLeft']

        # Calculate speeds
        df['KMPH_FrontRight'] = df['CMA_RPM_FrontRight'] * 0.0766
        df['KMPH_FrontLeft'] = df['CMA_RPM_FrontLeft'] * 0.0766
        df['AverageSpeed'] = (df['KMPH_FrontRight'] + df['KMPH_FrontLeft']) / 2
                ...
                try:
                    ...
                    try:
                        ...
                        with data_lock:
                            ...
                            # Keep only last N rows to prevent unlimited growth
                            if len(data) > 1000:  # adjust number as needed
                                data = data.iloc[-1000:]
...
def main():
    ...
    def gui():
        ...
        # Add safety check at the start of gui()
        if len(data) == 0:
            slider_value[0] = 0
        ...
        if imgui.button("Toggle CSV / Serial"):
            if use_serial[0]:
                stop_serial_reading()
                use_serial[0] = False
                auto_follow[0] = False  # Turn off auto-follow when switching to CSV
                print("Switched to CSV mode")
            else:
                if SERIAL_PORT and BAUD_RATE:  # Only switch if port and baud are set
                    use_serial[0] = True
                    auto_follow[0] = True  # Turn on auto-follow when switching to Serial
                    data = pd.DataFrame(columns=columns)
