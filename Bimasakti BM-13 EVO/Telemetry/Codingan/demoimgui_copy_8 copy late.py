            ...
            while not stop_serial_event.is_set():
                try:
                    ...
                    try:
                        ...
                        with data_lock:
                            if len(data) == 0:
                                data = process_dataframe(df_row)
                            else:
                                # Append and process only new data
                                data = pd.concat([data, process_dataframe(df_row)], ignore_index=True)
                            
                            # Keep only last N rows if needed
                            if len(data) > 10000:  # Adjust buffer size as needed
                                data = data.iloc[-5000:]
...
def main():
    ...
    def gui():
        ...
        if len(data) > 0:  # Only try to plot/show data if we have any
            ...
            if len(data) > 0 and current_idx < len(data):
                derajat_value = data['Derajat'].iloc[current_idx]
            ...
            if len(data) > 0 and current_idx < len(data):
                for label in suspension_labels:
                    ...
                    value = data[label].iloc[current_idx]
            ...
            if len(data) > 0 and current_idx < len(data):
                ...
                #TPS
                imgui.begin("TPS")
                value = data['TPS'].iloc[current_idx]
                ...
                #RPM
                imgui.begin("RPM")
                value = data['RPM'].iloc[current_idx]
                ...
                draw_list = imgui.get_window_draw_list()
                target_speed = data['AverageSpeed'].iloc[current_idx]
                speedometer1.render(draw_list, target_speed)
                imgui.end()

                imgui.begin("GPS Speed")
                draw_list = imgui.get_window_draw_list()
                target_speed = data['GPSS'].iloc[current_idx]
